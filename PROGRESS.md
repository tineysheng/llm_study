...
- [ ] 增加 `/help` 命令，展示可用指令
- [ ] 增加 `/reset` 命令，清空对话历史但保留 system prompt
- [ ] 增加 `/history` 命令，查看当前上下文消息数量和角色
- [ ] 支持输入 `quit` 或 `/exit` 退出程序

### 1.3 参数与稳定性

- [ ] 支持通过命令行参数或环境变量配置 model
- [ ] 支持配置 temperature，观察 0 / 0.7 / 1.5 的输出差异
- [ ] 支持配置 max tokens，理解输出长度和成本控制
- [ ] 给 API 请求增加 timeout，避免网络卡死
- [ ] 处理 scanner 错误和流式响应异常

### 1.4 结构化输出

- [ ] 让模型按 JSON 格式返回一个固定结构
- [ ] 用 Go `encoding/json` 解析模型输出
- [ ] 处理 JSON 解析失败的情况，并记录失败原因
- [ ] 总结结构化输出在业务系统里的用途和风险

### 1.5 阶段完成标准

- [ ] 能解释“LLM API 为什么是无状态的”
- [ ] 能解释多轮对话历史为什么会影响成本和延迟
- [ ] 能解释 streaming 和普通响应的区别
- [ ] 能在 AI 辅助下实现一个最小 LLM CLI，并能解释关键代码
- [ ] 在笔记中写下面试版总结：`notes/01-core-concepts.md`
